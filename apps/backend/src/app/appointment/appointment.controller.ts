...
export class AppointmentsController {
  ...
  @Get('doctor/:idDoctor')
  @ApiOperation({
    summary: 'Все слоты к doctorid забронированные status = waiting',
  })
  @ApiParam({
    name: 'idDoctor',
    required: true,
    ...
    try {
      ...
      if (!_appointment) {
        res
          ...
          .send('The appointmnent with this id doesn`t exist');
      }
  ...
  async create(
    ...
    try {
      ...
      res
        ...
        .send(
          `${result.patient.firstname} ${result.patient.lastname}, you booked appointment on date ${result.date_start} to Dr. ${result.doctor.user.lastname}`
        );
    } catch (error) {
      throw new BadRequestException(
        `An error occurred while booking. Error: ${error}`
      );
    }
  }
    ...
    try {
      ...
      if (!_app) {
        res
          ...
          .send(`The appointment with id=${id} doesn't exist.`);
      }
  ...
  async updateAppNewVisit(
    ...
    try {
      ...
      if (!_doctor) {
        res
          .status(HttpStatus.NOT_FOUND)
          .send(`The doctor with id=${newApp.doctor_id} doesn't exist`);
      }
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden.Произошла ошибка,удаление не возможно',
  })
